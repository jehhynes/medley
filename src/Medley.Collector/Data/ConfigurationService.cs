...
public class ConfigurationService
{
    ...
    public bool GetGoogleFeaturesEnabled()
    {
        try
        {
            var baseDirectory = AppDomain.CurrentDomain.BaseDirectory;
            var appSettingsPath = Path.Combine(baseDirectory, "appsettings.json");
            var developmentSettingsPath = Path.Combine(baseDirectory, "appsettings.Development.json");

            // Start with base settings
            var googleFeaturesEnabled = false;

            if (File.Exists(appSettingsPath))
            {
                var json = File.ReadAllText(appSettingsPath);
                var config = System.Text.Json.JsonSerializer.Deserialize<AppSettings>(json);
                googleFeaturesEnabled = config?.GoogleFeaturesEnabled ?? false;
            }

            // Override with development settings if they exist
            if (File.Exists(developmentSettingsPath))
            {
                var json = File.ReadAllText(developmentSettingsPath);
                var config = System.Text.Json.JsonSerializer.Deserialize<AppSettings>(json);
                if (config != null)
                {
                    googleFeaturesEnabled = config.GoogleFeaturesEnabled;
                }
            }

            return googleFeaturesEnabled;
        }
        catch
        {
            return false;
    ...
    {
        public bool GoogleFeaturesEnabled { get; set; }
    }
}